...
WeatherParameter = Literal[
    ...
    "wind", "windGust",
    "lclouds", "mclouds", "hclouds", "cbase", "visibility",
    "cape", "ptype", "weatherWarnings",
]

WaveParameter = Literal["waves", "windWaves", "wavesPower", "swell1", "swell2"]

AirQualityParameter = Literal[
    "aqi", "so2sm", "dustsm", "cosc", "go3", "no2", "pm10", "pm2p5",
    "pollenAlder", "pollenBirch", "pollenGrass",
    "pollenMugwort", "pollenOlive", "pollenRagweed",
]
...
async def _call_forecast(client: WindyClient, request: ForecastRequest, model_name: str) -> dict:
    try:
        response = await client.get_forecast(request)
        return _build_response(response)
    except WindyNoContentError:
        raise ValueError(
            f"No data available for model '{model_name}' with the requested parameters at this location"
        )
    except WindyBadRequestError as e:
        raise ValueError(f"Invalid request: {e}")
    except WindyServerError as e:
        raise RuntimeError(f"Windy API server error: {e}")
...
async def get_weather_forecast(
    ...
    Args:
        ...
        parameters: Weather parameters to fetch. Choose from temperature, wind, precipitation,
            clouds, visibility, and atmospheric instability indicators.
...
async def get_wave_forecast(
    ...
    """Get an ocean wave forecast for a specific location from the Windy point forecast API.
    ...
    Args:
        lat: Latitude in decimal degrees (-90 to 90).
        lon: Longitude in decimal degrees (-180 to 180).
        parameters: Wave parameters to fetch (wave height, wind waves, swell, wave power).
        ctx: Injected server context.
...
async def get_air_quality_forecast(
    ...
    """Get an air quality and pollen forecast for a specific location from the Windy point forecast API.

    Returns forecast time series for the requested air quality or pollen parameters. Only
    the parameters you explicitly list are fetched — request only what you need.

    Args:
        lat: Latitude in decimal degrees (-90 to 90).
        lon: Longitude in decimal degrees (-180 to 180).
        parameters: Air quality or pollen parameters to fetch (AQI, particulates, gases, pollen types).
        ctx: Injected server context.
